...
const options = { platform: "macos" as const, mode: "standard" as const, inNav: false, sectionCount: 3 };
...
describe("settingsKeyAction", () => {
  test("Escape and Mod+W close the window", () => {
    ...
    expect(settingsKeyAction("Ctrl+W", { ...options, platform: "windows" })).toEqual({ kind: "close" });
  ...
  test("arrows move only inside the section list", () => {
    ...
    expect(settingsKeyAction("<Down>", { ...options, inNav: true })).toEqual({ kind: "move", delta: 1 });
    expect(settingsKeyAction("<Up>", { ...options, inNav: true })).toEqual({ kind: "move", delta: -1 });
  });

  test("j and k move only in Yazi mode", () => {
    expect(settingsKeyAction("j", { ...options, inNav: true })).toBeNull();
    expect(settingsKeyAction("j", { ...options, inNav: true, mode: "yazi" })).toEqual({ kind: "move", delta: 1 });
    expect(settingsKeyAction("k", { ...options, inNav: true, mode: "yazi" })).toEqual({ kind: "move", delta: -1 });
  });
});
