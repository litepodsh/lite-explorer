...
export const COLUMN_LABELS: Record<ListColumn, string> = { name: "Name", type: "Type", size: "Size", date: "Created", modified: "Modified" };
...
export function loadColumns(path: string, storage: StorageAccess = localStorage): ListColumn[] {
  ...
  const valid = [...new Set(saved.filter((column): column is ListColumn => DEFAULT_COLUMNS.includes(column)))];
  return [...valid, ...DEFAULT_COLUMNS.filter(column => !valid.includes(column))];
}
export function saveColumns(path: string, columns: ListColumn[], storage: StorageAccess = localStorage) {
  if (!path) return;
  const all = new Map(Object.entries(read(storage)));
  all.delete(folderKey(path));
  all.set(folderKey(path), columns);
  try { storage.setItem(KEY, JSON.stringify(Object.fromEntries([...all].slice(-500)))); } catch { /* Keep the current visit usable. */ }
}
export function moveColumn(columns: ListColumn[], from: ListColumn, to: ListColumn): ListColumn[] {
  const next = [...columns];
  const index = next.indexOf(from), target = next.indexOf(to);
