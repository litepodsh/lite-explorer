...
describe("folder column order", () => {
  ...
  test("repairs duplicate, missing and invalid columns", () => {
    const storage = memory();
    storage.setItem("folder-column-order", JSON.stringify({ "/a": ["size", "size", "bogus", "name"] }));
  ...
  test("creation and modification sort independently", () => {
    const a = { name: "a", path: "/a", is_directory: false, is_hidden: false, created: 1, modified: 20 };
